...
class BuilderMixin
{
    ...
    public function whereJsonOverlaps(): Closure
    {
        return function ($column, $value, $boolean = 'and', $not = false) {
            ...
            if (!$value instanceof Expression) {
                $binding = method_exists($this->grammar, 'prepareBindingForJsonOverlaps')
                    ? $this->grammar->prepareBindingForJsonOverlaps($value)
