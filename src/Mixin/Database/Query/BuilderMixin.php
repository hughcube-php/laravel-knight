...
class BuilderMixin
{
     ...
     *   // SQL: WHERE JSON_OVERLAPS(tags, '["php","laravel"]')
     *
     *   // 支持 JSON 路径语法
    ...
    /**
     * 添加 OR JSON_OVERLAPS 查询条件.
     *
     * 示例:
     *   $query->where('status', 'active')
     *         ->orWhereJsonOverlaps('tags', ['vip', 'premium']);
     *   // SQL: WHERE status = 'active' OR JSON_OVERLAPS(tags, '["vip","premium"]')
     *
     ...
     *
     * 示例:
     *   $query->whereNotJsonOverlaps('tags', ['banned', 'deprecated']);
     *   // SQL: WHERE NOT JSON_OVERLAPS(tags, '["banned","deprecated"]')
     *
     * @return Closure(string $column, mixed $value, string $boolean = 'and'): static
     */
    public function whereNotJsonOverlaps(): Closure
    {
    ...
    /**
     * 添加 OR NOT JSON_OVERLAPS 查询条件.
     *
     * 示例:
     *   $query->where('status', 'active')
     *         ->orWhereNotJsonOverlaps('tags', ['banned']);
     *   // SQL: WHERE status = 'active' OR NOT JSON_OVERLAPS(tags, '["banned"]')
     *
     * @return Closure(string $column, mixed $value): static
     */
    public function orWhereNotJsonOverlaps(): Closure
    {
        return function ($column, $value) {
            /** @phpstan-ignore-next-line */
